...
export function useMonthlyActiveEmployeeIds(monthKey?: string) {
  ...
  const q = useQuery({
    ...
    queryFn: async (): Promise<MonthlyActiveEmployeeIdsResult> => {
      ...
      (ordersRes.data ?? []).forEach((r) => { if (r.employee_id) { ids.add(r.employee_id); orderIds.add(r.employee_id); } });
      ...
      return { monthKey: mk, employeeIds: ids, orderEmployeeIds: orderIds };
    },
    staleTime: 60_000,
    enabled,
  });

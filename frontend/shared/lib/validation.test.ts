...
describe('validation', () => {
  ...
  it('rejects empty string in validateEmail', () => {
    expect(validateEmail('')).toBe(false);
  });

  it('rejects empty string in validatePhoneNumber', () => {
    expect(validatePhoneNumber('')).toBe(false);
  });

  it('accepts valid phone number with Saudi country code', () => {
    expect(validatePhoneNumber('(966) 551-2345')).toBe(true);
  });

  it('rejects Saudi local format as invalid', () => {
    expect(validatePhoneNumber('0551234567')).toBe(false);
  });

  it('rejects empty string in validateNationalID', () => {
    expect(validateNationalID('')).toBe(false);
  });

  it('validateUploadFile rejects zero-byte file', () => {

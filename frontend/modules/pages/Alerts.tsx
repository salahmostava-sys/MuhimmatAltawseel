...
/** تنبيهات موظفين المخزَّنة في جدول alerts فقط (معرّفات UUID)، وليس التنبيهات المولَّدة مثل absconded-… */
const isDbBackedEmployeeAlertType = (type: string, id: string) => {
  if (!DB_BACKED_EMPLOYEE_ALERT_TYPES.has(type)) return false;
...
const Alerts = () => {
  ...
  const handleResolve = async () => {
    ...
    if (isDbBackedEmployeeAlertType(resolveDialog.type, resolveDialog.id)) {
      try {
  ...
  const handleDefer = async () => {
    ...
    // Persist DB-backed employee alerts
    if (isDbBackedEmployeeAlertType(deferDialog.type, deferDialog.id)) {
  ...
  const typeOptions = ['all', 'residency', 'insurance', 'authorization', 'probation', 'health_insurance', 'driving_license', 'platform_account', 'employee_absconded', 'employee_terminated'];
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-xl">
              نافذة المستندات والمنصات: خلال {iqamaAlertDays} يومًا من اليوم (تُضبط من إعدادات المشروع — أيام تنبيه الإقامة/المنصات).
            </p>
          </div>
          <DropdownMenu>

...
BEGIN
  ...
  FOR v_emp IN
    SELECT e.id FROM public.employees e WHERE e.status = 'active'
  LOOP
    v_total_orders := 0;
    ...
    FOR v_app IN
      ...
      FROM public.apps a WHERE a.is_active = true
    LOOP
      v_app_orders := 0;
      v_app_shift_days := 0;
      v_app_earnings := 0;
    ...
    WHERE ed.employee_id = v_emp.id AND ed.apply_month = p_month_year AND ed.approval_status = 'approved';
    ...
    WHERE ad.employee_id = v_emp.id AND ai.month_year = p_month_year AND ai.status IN ('pending', 'deferred');

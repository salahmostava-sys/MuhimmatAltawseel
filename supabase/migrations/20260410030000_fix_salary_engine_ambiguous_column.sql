-- Fix "column reference employee_id is ambiguous" in calculate_salary_for_employee_month.
-- The RETURNING clause references columns that clash with the RETURNS TABLE output params.
-- Solution: Use explicit table-qualified references and assign INTO output variables.
...
BEGIN
  ...
  -- Use direct variable assignment instead of RETURNING INTO to avoid
  -- "column reference employee_id is ambiguous" between table columns
  -- and RETURNS TABLE output parameters.
  ...
  manual_deduction   := v_manual_deduction;
  net_salary         := v_net;
...
COMMENT ON FUNCTION public.calculate_salary_for_employee_month IS
  'v5.1: Fix ambiguous column reference in RETURNING clause';
